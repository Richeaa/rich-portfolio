...
const skillCategories = [
  {
    ...
    skills: [
      ...
      { name: "Vite", icon: <img src={vite} alt="Vite" className="w-8 h-8" /> },
    ],
  },
  {
    title: "Backend & CMS",
    ...
    skills: [
      ...
      { name: "Firebase", icon: <img src={firebase} alt="Firebase" className="w-8 h-8" /> },
    ],
  },
  {
    title: "Database",
    ...
    skills: [
      ...
      { name: "SQL Server", icon: <img src={sqlserver} alt="Sql Server" className="w-8 h-8" /> },
    ],
  },
  {
    ...
    skills: [
      ...
      { name: "Keras", icon: <img src={keras} alt="Keras" className="w-8 h-8" /> },
      { name: "Streamlit", icon: <SiStreamlit />, color: "text-red-500" },
      { name: "Yolo", icon: <img src={yolo} alt="Yolo" className="w-8 h-8" /> },
      { name: "OpenCV", icon: <img src={opencv} alt="OpenCV" className="w-8 h-8" /> },
      { name: "Scikit-Learn", icon: <img src={scikit} alt="Scikit-Learn" className="w-8 h-8" /> },
    ],
  },
  {
  ...
  },
  {
    ...
    skills: [
      { name: "Telerik Test Studio", icon: <img src={telerik} alt="Telerik Test Studio" className="w-8 h-8" /> },
    ],
  },
  {
    ...
    skills: [
      { name: "Lucid Chart", icon: <img src={lucid} alt="Lucid Chart" className="w-8 h-8" /> },
...
const Skills = () => {
  return (
    <section id="skills" className="container mx-auto px-4 md:px-6 py-20 md:py-28">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
        {skillCategories.map((category, categoryIndex) => (
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.5, delay: (categoryIndex % 2) * 0.1 }}
            onMouseMove={handleSpotlight}
            className={`bento-card p-6 md:p-7 ${
              category.skills.length > 6 ? "md:col-span-2" : ""
            }`}
          >
            <div className="relative z-10 flex items-center gap-3 mb-5">
              ...
              <span className="ml-auto text-[11px] font-mono text-slate-600 tabular-nums">
            ...
            <div className="relative z-10 flex flex-wrap gap-2">
              {category.skills.map((skill) => (
                <div
                  key={skill.name}
                  className="group flex items-center gap-2 bg-surface-700/50 hover:bg-surface-700 border border-surface-border hover:border-brand-500/40 rounded-full pl-2 pr-3.5 py-1.5 transition-colors duration-200"
                >
                  <span className={`text-lg flex items-center justify-center ${skill.color || "text-slate-400"}`}>
                    {skill.icon}
                  </span>
